"""Atomically augment historical backfill rows with pre-open Asian index data."""
...
LOGGER = logging.getLogger("asia_market_history")
...
def update_asia_market_history(path: Path = INPUT_PATH) -> dict[str, Any]:
    ...
    for name in YAHOO_SERIES:
        dates = [row[f"{name}_trade_date"] for row in output]
        source_ranges[name] = {"start": min(dates), "end": max(dates)}
    output_fields = (
...
def main() -> int:
    ...
    try:
        result = update_asia_market_history()
    except (OSError, ValueError) as exc:
        LOGGER.error("Asia market history update failed: %s", exc)
        return 1
    LOGGER.info(
        "Asia market history updated | rows=%d | %s to %s",
