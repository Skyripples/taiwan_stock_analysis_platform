...
OUTPUT_PATH = PROJECT_ROOT / "data" / "analysis" / "walk_forward_validation.json"
...
def run_walk_forward_validation(
    ...
    for prediction_index in range(initial_training_size, len(rows)):
...
def _metrics(rows: list[dict[str, Any]]) -> dict[str, float]:
    ...
    if len(set(actual)) != 2:
        raise ValueError("Walk-forward outcomes must contain both classes for ROC AUC")
    return {
        ...
        "baseline_accuracy": _clean(sum(row["baseline_hit"] for row in rows) / len(rows)),
    }


def _accuracy(rows: list[dict[str, Any]]) -> float:
    return _clean(sum(row["hit"] for row in rows) / len(rows))


def _window_accuracy(rows: list[dict[str, Any]], window: int) -> dict[str, Any]:
    selected = rows[-window:]
    return {"sample_count": len(selected), "accuracy": _accuracy(selected)}


def _clean(value: float) -> float:
    result = float(value)
    if not math.isfinite(result):
        raise ValueError("Walk-forward metric is not finite")
